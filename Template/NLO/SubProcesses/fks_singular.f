...
c to the list of weights using the add_wgt subroutine
      ...
      call sborn_amp(p_born,ret_amp2,ret_jamp2,ret_amp_split,ret_amp_split_cnt,wgt_c,ret_ans_cnt)
...
C      gluon in the initial state
      ...
      ! compute the born
c      call sborn(p_born,wgtborn)
      call sborn_amp(p_born,ret_amp2,ret_jamp2,ret_amp_split,ret_amp_split_cnt,wgtborn,ret_ans_cnt)
      alphas = g**2/4d0/pi
      do iamp = 1, amp_split_size
        if (ret_amp_split(iamp).eq.0d0) cycle
        call amp_split_pos_to_orders(iamp, orders)
...
c Compute the multi-channel enhancement factor 'enhance'.
      ...
      if (p_born(0,1).gt.0d0) then
         call sborn_amp(p_born,amp2,jamp2,ret_amp_split,ret_amp_split_cnt,wgt_c,dummy_ans)
...
c use the Born computed with those as the mapping.
      ...
      if ((granny_is_res .or. .not.use_evpr).and. imode.eq.2) then
         ...
         if (p_born_ev(0,1).gt.0d0) then
            ...
            call set_alphas(pas)
            call sborn_amp(p_born_used,amp2,jamp2,ret_amp_split,ret_amp_split_cnt,wgt_c,dummy_ans)
...
c has soft singularities
         ...
         amp_split(1:amp_split_size) = ret_amp_split(1:amp_split_size)
      endif

      ret_amp_split(1:amp_split_size) = amp_split(1:amp_split_size)
...
c Unphysical kinematics: set matrix elements equal to zero
      ...
      t = z * shat/4d0
      call sborn_amp(p_born,amp2,jamp2,ret_amp_split,ret_amp_split_cnt,wgt_born,ans_cnt)
      if (iextra_cnt.gt.0)
...
C check if any extra_cnt is needed
         if (iextra_cnt.gt.0) then
            ...
            ! this is the contribution from the born ME
               call sborn_amp(p_born_used,amp2,jamp2,ret_amp_split,ret_amp_split_cnt,wgt_born,ans_cnt)
...
c
      call sborn_amp(p_born(0,1),amp2,jamp2,RET_AMP_SPLIT,DUMMY_AMP_SPLIT_CNT,wgt1,ans_cnt)
c
...
c entering this function
      ...
      call sborn_amp(p_born,amp2,jamp2,ret_amp_split,ret_amp_split_cnt,wgt1,ans_cnt)
